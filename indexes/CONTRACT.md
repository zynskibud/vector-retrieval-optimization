...
1. After **every** build, with any thread count, run a **repair pass**: find every node with zero incoming edges on layer 0. (A one-thread build also leaves a few such nodes, 7 of 100,000 in Rust, because Algorithm 4 with `keepPrunedConnections = false` can prune every in-edge of a node. The repair pass makes the result the same in both cases: every node reachable.) For each such node v, take u = the nearest node in v's own layer-0 list (or, if v has no out-edges, the nearest node from a fresh search-layer with `ef_construct`), add the edge u → v, and if u's list exceeds its cap, shrink it with the heuristic but with v kept (v is not a candidate for pruning). Repeat until no node has zero in-degree on layer 0 (one pass is normally enough; cap at 3 passes and report the count in `extra.repair_added`).
2. Tests must show that a build with all cores and a build with `--threads 1` both reach **every** node from the entry point on layer 0 by BFS (100%, not 99%), and that the two builds' recall@10 at ef=64 is within 0.01 on the same 20,000 rows.
