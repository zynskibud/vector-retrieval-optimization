...
- **One thread means one thread in every library too.** Python must set `VECLIB_MAXIMUM_THREADS`, `OPENBLAS_NUM_THREADS`, `OMP_NUM_THREADS`, and `MKL_NUM_THREADS` to 1 before NumPy loads. FAISS sets `omp_set_num_threads(1)` for the timed loop. Note that NumPy's BLAS on macOS is Apple Accelerate, which runs matrix products on the AMX unit even on one thread, so Python's flat scan is fast for a reason that has nothing to do with Python. The report must say this.
...
1. After **every** build, with any thread count, run a **repair pass**. Two defects must be fixed: a node with zero layer-0 in-degree (Algorithm 4 with `keepPrunedConnections = false` can prune every in-edge of a node; a one-thread build on 100k rows leaves about 7 of them), and a group of near-duplicates that only link to each other, so every member has in-degree ≥ 1 but no edge enters the group from the rest of the graph (a parallel build leaves a few such groups). One pass, in this order:
   - **Step A, in-degree.** For each node v with zero layer-0 in-degree, in row order, skipping the entry point: u = the nearest node in v's own layer-0 list, or, if v has no out-edges, the nearest result of a search-layer from the entry point with `ef_construct`. Add the edge u → v. If u's list now exceeds its cap, shrink it with the heuristic, but v and every edge added earlier by the repair are **protected**: they are never pruned.
   - **Step B, reachability.** Directed BFS from the entry point on layer 0. For each node v that the BFS did not reach, in row order: run a search-layer from the entry point with `ef_construct` (it visits reachable nodes only). Among its results, nearest first, u = the first node whose layer-0 list has a free slot; if none has one, u = the nearest. Add u → v, protected as in step A, with the same shrink rule.
   - Repeat the pass while step B still found an unreachable node, at most 3 passes. Report how many nodes step A and step B touched.
   The repair is part of the build and is included in `add_s`.
2. Tests must show that a build with all cores and a build with `--threads 1` both reach **every** node from the entry point on layer 0 by BFS (100%, not 99%), and that the two builds' recall@10 at ef=64 is within 0.01 on the same 20,000 rows. A parallel build is not deterministic (the repair counts vary from run to run); that is accepted. Parallel workers should claim rows in chunks of 64 consecutive rows, so that near-duplicate neighbors in consecutive rows are inserted in order by one worker; without this, Go measured a 0.016 recall gap to the one-thread build even at 100% reachability.
3. `extra` reports `top_layer`, `entry_point`, `nodes_per_layer` (as a list, or as `nodes_layer_<l>` keys where the language's extra map holds only numbers), `unreachable_before_repair` (the BFS count before any repair), `repair_added` (all edges added by the repair), `repair_added_unreachable` (edges from step B), and `build_threads`.
...
3. Two passes over all rows in row order, first with alpha = 1.0, then with `alpha`. For row i: greedy search from the entry point with list size `l_build` to get the visited set V; robust-prune(i, V, alpha, r); for each new out-neighbor j of i, add the edge j→i and, if j now has more than r edges, robust-prune j.
   **Robust prune** (Algorithm 2 of the paper) uses the squared Euclidean distance of unit vectors, `d(a, b) = 2 − 2·(a·b)`. The candidate set is V ∪ N_out(i) minus i itself, as in the paper. Sort the candidates by d(i, p) ascending, lower ID first on ties; take the nearest p* into the list; remove every remaining p with `alpha · d(p*, p) ≤ d(i, p)`; repeat until the list has r nodes or no candidates remain. Parallel builds may use 64-row chunks and a lock per node as in HNSW; `--threads 1` is strictly row order.
